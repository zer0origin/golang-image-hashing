...
func (h Hash) ConvertByteArrToNumberFNV1A(bytes []byte) uint64 {
...
//func (h Hash) ConvertByteArrToNumberFNV1ABase64Encoded(encodedData string) uint64 {
...
//	}
//
//	return h.ConvertByteArrToNumberFNV1A(bytes)
//}
...
//func (h Hash) ConvertByteArrToNumberSum(bytes []byte) uint64 {
//	var k uint64 = 0
//	for _, byt := range bytes {
//		k = k + uint64(byt)
//	}
...
func (h Hash) HashPageOfDocument(bytes []byte) uint64 {
	i := h.ConvertByteArrToNumberFNV1A(bytes)
	return h.Hash64shift(i)
}
...
func (h Hash) Hash64shift(key uint64) uint64 {
	key = (key) + (key << 21)
	key = key ^ (key >> 24)
...
func (h Hash) ConvertHashToString(input uint64) string {
	s := ""
